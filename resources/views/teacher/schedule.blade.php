        ...
        <div style="display:flex; gap:0.75rem; align-items:center; flex-wrap:wrap;">
            ...
            @if(!empty($detectedClasses) && $detectedClasses->count()>0)
                <div style="margin-left:0.75rem; display:flex; gap:0.5rem; align-items:center; flex-wrap:wrap;">
                    <div style="font-size:0.85rem; color:var(--text-secondary); font-weight:600;">Terdeteksi di jadwal:</div>
                    @foreach($detectedClasses as $dc)
                        <a href="?class={{ $dc }}" class="qs-btn">Kelas {{ $dc }}</a>
                    @endforeach
                    {{-- Import form (POST) for teacher to import parsed items into DB --}}
                    <form method="post" action="{{ route('teacher.schedule.import_parsed') }}" style="margin-left:0.5rem;">
                        @csrf
                        <input type="hidden" name="class" value="{{ $detectedClasses->first() }}">
                        <button type="submit" class="qs-btn" style="background:#dff3e6;">Import ke DB (kelas {{ $detectedClasses->first() }})</button>
                    </form>
                </div>
            @endif
            {{-- Button removed: auto-submit on change --}}
            @if(!empty($selectedClass))
                <div style="margin-left:auto; font-weight:600; color:var(--text-secondary);">Jadwal Mengajar Kelas {{ $selectedClass }}</div>
            @endif
        </div>
    </form>
