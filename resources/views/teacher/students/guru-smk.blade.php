@php
    $smkClassesList = $smkSchedules->keys();
    $firstSMKClass = $smkClassesList->first();
@endphp
...
@if($firstSMKClass)
    ...
    <div class="card card-custom mb-4">
        <div class="card-body p-4">
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
                <div>
                    <h5 class="fw-bold text-muted mb-1" style="font-size: 0.9rem; text-transform: uppercase; letter-spacing: 0.5px;">🎓 Guru SMK</h5>
                    <h2 class="fw-bold mb-1" style="color: #2F5D50; font-family: 'Poppins', sans-serif;" id="smk-active-class-title">Kelas {{ $firstSMKClass }}</h2>
                    <p class="text-secondary mb-0"><i class="fas fa-users me-2"></i><strong id="smk-active-student-count">{{ $smkSchedules->get($firstSMKClass)->count() }}</strong> Siswa yang diajar</p>
                </div>
                <div>
                    <div class="d-flex align-items-center gap-2">
                        <label class="fw-bold text-secondary mb-0 me-2" style="font-size: 0.95rem;">Pilih Kelas:</label>
                        <select class="form-select smk-class-select" style="min-width: 140px; border-radius: 10px; border-color: #E2E8F0; padding: 8px 12px; font-weight: 600;" onchange="switchSMKClass(this.value)">
                            @foreach($smkClassesList as $smkClass)
                                <option value="{{ $smkClass }}" data-count="{{ $smkSchedules->get($smkClass)->count() }}">{{ $smkClass }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
    ...
    <!-- Class containers -->
    @foreach($smkSchedules as $className => $classStudents)
        <div class="smk-class-container" id="smk-class-container-{{ $className }}" style="display: {{ $className === $firstSMKClass ? 'block' : 'none' }};">
            <!-- Table Card -->
    ...
    <script>
        function switchSMKClass(className) {
            // Hide all containers
            document.querySelectorAll('.smk-class-container').forEach(el => {
                el.style.display = 'none';
            });
            
            // Show target container
            const target = document.getElementById('smk-class-container-' + className);
            if (target) {
                target.style.display = 'block';
            }
            
            // Update title
            document.getElementById('smk-active-class-title').innerText = 'Kelas ' + className;
            
            // Update count
            const selectEl = document.querySelector('.smk-class-select');
            const selectedOption = selectEl.options[selectEl.selectedIndex];
            const studentCount = selectedOption.getAttribute('data-count');
            document.getElementById('smk-active-student-count').innerText = studentCount;
        }
    </script>
@else
    <!-- Empty Role Card -->
    <div class="card card-custom p-5 text-center">
        ...
        <p class="text-secondary mb-0">Anda tidak terdaftar sebagai Guru SMK.</p>
    </div>
@endif
