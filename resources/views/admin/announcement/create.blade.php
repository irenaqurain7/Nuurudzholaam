...
<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; border-bottom: 1px solid #eaedf1; padding-bottom: 15px;">
    <div>
        <h1 style="margin: 0; color: var(--hijau-islam); font-size: 28px; font-weight: 700;">Tambah Pengumuman</h1>
        <p style="margin: 5px 0 0 0; color: #718096; font-size: 14px;">Buat dan publikasikan pengumuman atau informasi baru untuk lingkungan sekolah.</p>
    </div>
...
<div style="max-width: 800px; margin: 0 auto;">
    <form method="POST" action="{{ route('admin.announcement.store') }}" style="background: white; padding: 35px; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05), 0 2px 4px -1px rgba(0,0,0,0.03); border: 1px solid #e2e8f0;">
        @csrf

        <div class="form-group">
            <label for="judul">Judul Pengumuman <span class="required">*</span></label>
            <input type="text" id="judul" name="judul" value="{{ old('judul') }}" placeholder="Contoh: Jadwal Pelaksanaan Libur Semester Akhir" required>
        </div>

        <div class="form-group">
            <label for="konten">Konten / Isi Pengumuman <span class="required">*</span></label>
            <textarea id="konten" name="konten" placeholder="Tuliskan detail pengumuman secara lengkap di sini..." required>{{ old('konten') }}</textarea>
        </div>
            ...
            <div class="form-group">
                ...
                <div class="select-wrapper">
                    <select id="status" name="status" required>
                        <option value="aktif" @if(old('status', 'aktif') === 'aktif') selected @endif>Aktif (Tampilkan)</option>
                        <option value="arsip" @if(old('status') === 'arsip') selected @endif>Arsip (Sembunyikan)</option>
                    </select>
                </div>
            </div>
        </div>
